...
public class NativeResource {
	...
	public synchronized void load(String libraryName, InputStream stream) throws Exception {
		if(loaded) {
			return;
		}
		loaded = true;
		...
		loadLib(libraryName, stream);
	}

	private void inJarLoad(String name, InputStream stream)throws Exception{
		//InputStream resourceSource = getClass().getResourceAsStream(fileName);
		if (stream == null){
		...
		try {
			copyResource(stream, resourceLocation);
		} catch (IOException e) {
	...
	private void loadLib(String libName, InputStream stream) throws Exception {
		try
		{
			inJarLoad(libName, stream);
	...
	public static String getFileName(String name) throws NativeResourceException
	{
		...
		if(OSUtil.isOSX()) {
			...
			throw new NativeResourceException("Can't load native file: "+name+" for os arch: "+OSUtil.getOsArch());
		}
		return file;
	}
