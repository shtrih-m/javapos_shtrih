...
public class NativeResource {
	...
	public synchronized void load(String libraryName) throws Exception {
		if(loaded) {
			return;
		}
		loaded = true;
	...
	private void inJarLoad(String name)throws Exception{
		//start by assuming the library can be loaded from the jar
		String fileName = getFileName(name);
		InputStream resourceSource = StaticContext.getContext().getAssets().open(fileName);
		//InputStream resourceSource = getClass().getResourceAsStream(fileName);
		if (resourceSource == null){
			throw new Exception(String.format("Embedded resource %s not found", fileName));
		}
	...
	private void loadLib(String libName) throws Exception {
		try
		{
			inJarLoad(libName);
	...
	private String getFileName(String name) throws NativeResourceException
	{
